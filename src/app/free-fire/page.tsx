...
export default function FreeFirePage() {
  ...
  useEffect(() => {
    fetch("/api/products?category=free-fire", { cache: "no-store" })
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div className="grid gap-6 page-transition">
      {/* Banner */}
      <section className="relative overflow-hidden rounded-3xl border border-rose-500/20 bg-gradient-to-br from-rose-500/10 via-transparent to-pink-500/5 p-6 md:p-10">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 rounded-full border border-rose-400/20 bg-rose-500/10 px-4 py-2 text-sm font-bold text-rose-300">
            <span>🔥</span>
            <span>Free Fire — حسابات نادرة</span>
          </div>
          <h1 className="title mt-4">حسابات Free Fire</h1>
      ...
      <SearchBar
        ...
        placeholder="ابحث في حسابات Free Fire..."
      />
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description={products.length === 0 ? "لم يتم إضافة أي حساب Free Fire بعد." : "حاول تغيير كلمات البحث"}
        />
      ) : (
