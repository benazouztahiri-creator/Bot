...
export async function GET() {
  ...
  return NextResponse.json(unmatchedItems);
}
...
export async function POST(req: Request) {
  ...
  if (!unmatched_payment_id) {
    return NextResponse.json({ error: "Missing unmatched_payment_id" }, { status: 400 });
  }
  ...
  if (selected_order_id) {
    ...
    await logAuditEvent({
      event_type: "order.payment_unmatched_resolved",
      user_id: user.id,
      order_id: selected_order_id,
      details: `Admin resolved unmatched payment ${unmatched.transaction_id} for order`,
    });
    ...
    await createNotification({
      ...
      message: "تم تأكيد دفع طلبك بعد المراجعة.",
  ...
  await db.execute(
    ...
    [selected_order_id || null, notes || "", unmatched_payment_id]
  );
