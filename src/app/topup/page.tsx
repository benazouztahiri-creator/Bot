...
export default function TopupPage() {
  ...
  useEffect(() => {
    fetch("/api/products?category=topup", { cache: "no-store" })
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div className="grid gap-6 page-transition">
      {/* Banner */}
      <section className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-transparent to-teal-500/5 p-6 md:p-10">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-500/10 px-4 py-2 text-sm font-bold text-emerald-300">
            <span>⚡</span>
            <span>Top-up — شحن فوري</span>
          </div>
          <h1 className="title mt-4">خدمات الشحن (Top-up)</h1>
          ...
          <div className="mt-4 flex flex-wrap gap-4">
            <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-center">
              ...
              <div className="text-xs text-white/50">إجمالي الخدمات</div>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-center">
      ...
      <SearchBar
        ...
        placeholder="ابحث في خدمات الشحن..."
      />
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon="⚡"
          title={products.length === 0 ? "لا توجد خدمات متاحة" : "لا توجد نتائج للبحث"}
          description={products.length === 0 ? "لم يتم إضافة أي خدمة شحن بعد." : "حاول تغيير كلمات البحث"}
        />
      ) : (
            ...
            <article key={p.id} className="product-card glass flex flex-col rounded-3xl">
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="text-xl font-black text-white">{p.price} USD</div>
                  {p.status === "sold" ? (
                    <span className="btn-secondary w-full cursor-not-allowed opacity-50 sm:w-auto text-center">تم البيع</span>
                  ) : (
                    <a className="btn-primary w-full sm:w-auto" href={`/products/${encodeURIComponent(p.id)}`}>اطلب الآن</a>
                  )}
                </div>
              </div>
            </article>
